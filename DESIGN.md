...
| Org | `org:{org_name}` | All members of the org. Shared. | `org:health-chain` |
| Project | `project:{project_name}` | Opt-in project scope. Shared subset. | `project:centaur-infra` |
...
results = await client.search(
    ...
    namespace=["personal:thameema", "org:health-chain", "project:centaur-infra"]
)
```
...
```
You are synthesizing results from {n} parallel worker agents.
...
```
User prompt: "Review the centaur-infra Terraform and flag any IAM misconfigurations"
                  │
                  ▼
            Orchestrator
